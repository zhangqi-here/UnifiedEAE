...
class PAIE(nn.Module):
    def __init__(self, config, path):
        super(PAIE, self).__init__()
        self.config = config
    ...
    def forward(
        ...
        for i, (context_output, decoder_prompt_output, arg_joint_prompt, old_tok_to_new_tok_index) in \
            enumerate(zip(context_outputs_shared, decoder_prompt_outputs_shared, arg_joint_prompts, old_tok_to_new_tok_indexs)):
        ...
        #----------------dataset_a----------------
        if torch.sum(dataset_ids == 0) > 0:
            weight_a = torch.sigmoid(
                self.w_a(torch.cat((context_outputs_dataset_a, context_outputs_shared[dataset_ids == 0]), dim=-1)))
            context_outputs_dataset_a = weight_a * context_outputs_dataset_a + (1 - weight_a) * context_outputs_shared[
                dataset_ids == 0]
            weight_decoder_a = torch.sigmoid(self.w_decoder_a(
                torch.cat((decoder_prompt_outputs_dataset_a, decoder_prompt_outputs_shared[dataset_ids == 0]), dim=-1)))

            decoder_prompt_outputs_dataset_a = weight_decoder_a * decoder_prompt_outputs_dataset_a + (
                        1 - weight_decoder_a) * decoder_prompt_outputs_shared[dataset_ids == 0]
            # context_outputs_dataset_a = context_outputs_dataset_a + context_outputs_shared[dataset_ids==0]
            # decoder_prompt_outputs_dataset_a = decoder_prompt_outputs_dataset_a + decoder_prompt_outputs_shared[dataset_ids==0]
            for i, (context_output, decoder_prompt_output, arg_joint_prompt, old_tok_to_new_tok_index) in \
        ...
        if torch.sum(dataset_ids == 1) > 0:
            ...
            decoder_prompt_outputs_dataset_b = weight_decoder_b * decoder_prompt_outputs_dataset_b + (
                    1 - weight_decoder_b) * decoder_prompt_outputs_shared[dataset_ids == 1]
            # context_outputs_dataset_b = context_outputs_dataset_b + context_outputs_shared[dataset_ids == 1]
            # decoder_prompt_outputs_dataset_b = decoder_prompt_outputs_dataset_b + decoder_prompt_outputs_shared[dataset_ids == 1]
            for i, (context_output, decoder_prompt_output, arg_joint_prompt, old_tok_to_new_tok_index) in \
        ...
        for i in range(dataset_ids.size(0)):
            if dataset_ids[i].item() == 0:
                ...
                dataset_b_index += 1
        if self.training:
            total_loss_shared = total_loss_shared/len(context_outputs_shared)
            dataset_num = 0
            if torch.sum(dataset_ids==0) > 0:
                dataset_num += 1
            ...
            if torch.sum(dataset_ids == 1) > 0:
                ...
                total_loss_dataset_b = 0
            total_loss = total_loss_shared + total_loss_dataset_a + total_loss_dataset_b
...
class PAIE_VIB_AVG(nn.Module):
    def __init__(self, config ,path):
        super(PAIE_VIB_AVG, self).__init__()
        self.config = config
        self.model_shared = BartModel.from_pretrained(path)
        self.model_a = BartModel.from_pretrained(path)
        self.model_b = BartModel.from_pretrained(path)
    ...
    def forward(
        ...
        if torch.sum(dataset_ids == 0) > 0:
            context_outputs_dataset_a = context_outputs_dataset_a + context_outputs_shared[dataset_ids == 0]
            decoder_prompt_outputs_dataset_a = decoder_prompt_outputs_dataset_a + decoder_prompt_outputs_shared[
                dataset_ids == 0]
            for i, (context_output, decoder_prompt_output, arg_joint_prompt, old_tok_to_new_tok_index) in \
                    enumerate(zip(context_outputs_dataset_a, decoder_prompt_outputs_dataset_a, arg_joint_prompts_a,
                                  old_tok_to_new_tok_indexs_a)):
        ...
        if torch.sum(dataset_ids == 1) > 0:
            ...
            # print(context_outputs_dataset_b.size())
            context_outputs_dataset_b = context_outputs_dataset_b + context_outputs_shared[dataset_ids == 1]
            decoder_prompt_outputs_dataset_b = decoder_prompt_outputs_dataset_b + decoder_prompt_outputs_shared[
                dataset_ids == 1]
            for i, (context_output, decoder_prompt_output, arg_joint_prompt, old_tok_to_new_tok_index) in \
                    enumerate(zip(context_outputs_dataset_b, decoder_prompt_outputs_dataset_b, arg_joint_prompts_b,
                                  old_tok_to_new_tok_indexs_b)):
        ...
        if self.training:
            ...
            if torch.sum(dataset_ids == 1) > 0:
                ...
                total_loss_dataset_b = 0
            total_loss = self.alpha*total_loss_shared + total_loss_dataset_a + total_loss_dataset_b
            # print("&&"*10, total_loss, total_loss_shared, total_loss_dataset_a, total_loss_dataset_b, dataset_a_index, dataset_b_index, dataset_ids)
            # print(dataset_ids)
            return total_loss, logit_lists_shared, logit_lists_dataset
        else:
...
class PAIE_VIB(nn.Module):
    def __init__(self, config ,path):
        super(PAIE_VIB, self).__init__()
        self.config = config
        self.model_shared = BartModel.from_pretrained(path)
        self.model_a = BartModel.from_pretrained(path)
        self.model_b = BartModel.from_pretrained(path)
