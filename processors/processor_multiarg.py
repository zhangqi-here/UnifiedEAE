...
class InputFeatures(object):
    ...
    def __init__(self, example_id, feature_id, 
                ...
                old_tok_to_new_tok_index = None, full_text = None, arg_list=None, dataset_id=None
    ...
    def init_pred(self):
        self.pred_dict_tok_shared = dict()
        self.pred_dict_word_shared = dict()
        self.pred_dict_tok_dataset = dict()
        self.pred_dict_word_dataset = dict()

    def add_pred_shared(self, role, span, dset_type):
        if role not in self.pred_dict_tok_shared:
            self.pred_dict_tok_shared[role] = list()
        if span not in self.pred_dict_tok_shared[role]:
            self.pred_dict_tok_shared[role].append(span)

            if span!=(0, 0):
                if role not in self.pred_dict_word_shared:
                    self.pred_dict_word_shared[role] = list()
                word_span = self.get_word_span(span, dset_type)         # convert token span to word span 
                if word_span not in self.pred_dict_word_shared[role]:
                    self.pred_dict_word_shared[role].append(word_span)

    def add_pred_dataset(self, role, span, dset_type):
        if role not in self.pred_dict_tok_dataset:
            self.pred_dict_tok_dataset[role] = list()
        if span not in self.pred_dict_tok_dataset[role]:
            self.pred_dict_tok_dataset[role].append(span)

            if span != (0, 0):
                if role not in self.pred_dict_word_dataset:
                    self.pred_dict_word_dataset[role] = list()
                word_span = self.get_word_span(span, dset_type)  # convert token span to word span
                if word_span not in self.pred_dict_word_dataset[role]:
                    self.pred_dict_word_dataset[role].append(word_span)
...
class ArgumentExtractionDataset(Dataset):
    ...
    def collate_fn(batch):
        ...
        dataset_ids = torch.tensor([f.dataset_id for f in batch])
        ...
        return enc_input_ids, enc_mask_ids, \
                ...
                start_position_ids, end_position_ids, dataset_ids
...
class MultiargProcessor(DSET_processor):
    ...
    def _read_prompt_group(prompt_path):
        sub_prompt_path = prompt_path.split('#')
        prompts = dict()
        for path in sub_prompt_path:
            if path.find("ace") > -1:
                dataset = "ace"
            elif path.find("wiki") > -1:
                dataset = "wikievent"
            elif path.find("rams") > -1:
                dataset = "rams"
            else:
                dataset = None
            if dataset not in prompts:
                prompts[dataset] = dict()
            with open(path) as f:
                lines = f.readlines()
            # prompts = dict()
            for line in lines:
                ...
                prompts[dataset][event_type] = prompt
        return prompts
    ...
    def convert_examples_to_features(self, examples, dataset2id_map):
        ...
        for example_idx, example in enumerate(examples):
            ...
            dataset_id = dataset2id_map[example.dataset_name]
            ...
            if len(enc_input_ids) > self.args.max_enc_seq_length:
                enc_input_ids = enc_input_ids[:self.args.max_enc_seq_length]
                enc_mask_ids = enc_mask_ids[:self.args.max_enc_seq_length]
            ...
            # print("dataset_name: ", dataset_name, " event_type: ", event_type)
            # print("prompts: ", prompts[dataset_name])
            # Deal with prompt template
            if self.prompt_query:
                dec_prompt_text = prompts[dataset_name][event_type].strip()
                if dec_prompt_text:
            ...
            for arg in arg_list:
                ...
                if arg in event_args_name:
                    ...
                    for arg_idx in arg_idxs:
                        ...
                        # print ("start_old: ", start_old)
                        # print ("*****old_tok_to_new_tok_index: ", len(old_tok_to_new_tok_index))
                        start_position = old_tok_to_new_tok_index[start_old][0]
                        end_position = old_tok_to_new_tok_index[end_old-1][1]
                        if end_position >= self.args.max_enc_seq_length:
                            continue
            ...
            features.append(
                    InputFeatures(example_id, feature_idx, 
                                ...
                                dec_prompt_text, dec_prompt_ids, dec_prompt_mask_ids,
                                arg_quries, arg_joint_prompt, target_info,
                                old_tok_to_new_tok_index = old_tok_to_new_tok_index, full_text=example.full_text, arg_list = arg_list, dataset_id=dataset_id
                    )
            )
